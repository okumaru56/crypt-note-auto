...
def generate_article(news_text):
    # 新しい SDK のクライアント初期化 (GEMINI_API_KEY は環境変数から自動取得されます)
    client = genai.Client()

    # main.py と同じ階層にある prompt.txt のパスを取得
    ...
    # 429エラー対策のリトライ処理
    max_retries = 3
    retry_delay = 45  # 制限解除まで45秒待機

    for attempt in range(max_retries):
        try:
            # モデルは安定した gemini-2.5-flash または gemini-2.5-pro を推奨
